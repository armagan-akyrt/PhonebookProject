...
export function fetchPreviousMeetings(searchWord, isActive, startInterval, endInterval, userId) {
    fetch(`/getPreviousMeetings?searchWord=${searchWord}&isActive=${isActive}&startInterval=${startInterval}&endInterval=${endInterval}&userId=${userId}`)
    .then(response => response.json())
    .then(data => {
        let meetingList = document.getElementById('listToShowMeeting');
        meetingList.innerHTML = ''; // Clear the list before displaying new results
